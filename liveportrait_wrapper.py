...
from liveportrait_utils.timer import Timer
from liveportrait_utils.helper import load_model, concat_feat, load_mapping_network
from liveportrait_utils.camera import headpose_pred_to_degree, get_rotation_matrix
from liveportrait_utils.retargeting_utils import calc_eye_close_ratio, calc_lip_close_ratio
...
class LivePortraitWrapper(object):
    ...
    def __init__(self, inference_cfg: InferenceConfig):
        ...
        mapping_net_cfg = yaml.load(open(inference_cfg.mapping_net_cfg, 'r'), Loader=yaml.SafeLoader)
        # init F
        ...
        self.mapping_net, self.image_preprocessor = load_mapping_network(inference_cfg.checkpoint_X, OmegaConf.create(mapping_net_cfg), self.device)
    ...
    def prepare_videos_no_expand(self, imgs):
        if isinstance(imgs, list):
            _imgs = np.array(imgs)  # TxHxWx3
        elif isinstance(imgs, np.ndarray):
            _imgs = imgs
        else:
            raise ValueError(f'imgs type error: {type(imgs)}')
        y = _imgs.astype(np.float32) / 255.
        y = np.clip(y, 0, 1)  # clip to 0~1
        y = torch.from_numpy(y).permute(0, 3, 1, 2)  # TxHxWx3x1 -> Tx1x3xHxW
        y = y.to(self.device)
        return y
    ...
    def warp_decode(self, feature_3d: torch.Tensor, kp_source: torch.Tensor, kp_driving: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad(), self.inference_ctx():
            ...
            # warped_feature = ret_dct['out']
            # decode
            decode_out = self.spade_generator(feature=ret_dct['out'])
            ret_dct['out'] = decode_out
            
            # ret_dct['out'] = decode_out[0] if self.inference_cfg.flag_multitask_spade else decode_out
        ...
        return ret_dct#, warped_feature
    
    def get_control_values(self, images):
        processed_imgs = self.image_preprocessor(images) # [bs, 3, 224, 224]
        pos_pred_vals = self.mapping_net(processed_imgs)
