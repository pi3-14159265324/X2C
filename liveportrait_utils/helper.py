...
from models.liveportrait.spade_generator import SPADEDecoder
from models.liveportrait.warping_network import WarpingNetwork
from models.liveportrait.motion_extractor import MotionExtractor
from models.liveportrait.appearance_feature_extractor import AppearanceFeatureExtractor
from models.liveportrait.stitching_retargeting_network import StitchingRetargetingNetwork
...
def load_mapping_network(ckpt_path, cfg, device):
    model = X2Control(cfg).to(device)
    state_dict = torch.load(ckpt_path, map_location=lambda storage, loc: storage, weights_only=False)
    model.load_state_dict(state_dict['model'])
    model.eval()
    model.requires_grad_(False)
    print(f'mapping net path: {ckpt_path} \n ==========')
    mean, std = cfg.data.mean, cfg.data.std
    resize_target = cfg.data.transform.resize_target
    image_preprocessor = transforms.Compose(
			[transforms.Resize(resize_target), transforms.Normalize(mean=mean, std=std)]) 

    # image_preprocessor = Transform(cfg.data, split_type='val', physical=False, imagenet_pretrain=True) # ImageProcessor(cfg.data, split_type='val')
...
def load_model(ckpt_path, model_config, device, model_type):
    ...
    if model_type == 'appearance_feature_extractor':
        ...
        if 'retarget_shoulder' not in checkpoint:
            stitcher.load_state_dict(checkpoint)
            stitcher = stitcher.to(device)
            stitcher.eval()
            return {'stitching': stitcher}
    ...
    model.load_state_dict(torch.load(ckpt_path, map_location=lambda storage, loc: storage, weights_only=True))
    model.eval()
    return model
