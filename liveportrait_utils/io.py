...
def load_images_from_bytes(bytes_list):
    return [iio.imread(BytesIO(bytes)) for bytes in bytes_list]
...
def load_image_sequences(image_folder, max_len=-1):
    frames = os.listdir(image_folder)
    frames.sort()
    if max_len > 0:
        frames = frames[-max_len:]
    frames = [osp.join(image_folder, frame) for frame in frames]
    ret = [imageio.core.Array(iio.imread(frame)) for frame in frames]
    return ret
