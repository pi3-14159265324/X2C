...
class ArgumentConfig(PrintableConfig):
    ########## input arguments ##########
    source: Annotated[str, tyro.conf.arg(aliases=["-s"])] = make_abs_path('../datasets/example.jpg')
    
    driving:  Annotated[str, tyro.conf.arg(aliases=["-d"])] = make_abs_path('driving_video.mp4')  # 

    output_dir: Annotated[str, tyro.conf.arg(aliases=["-o"])] = make_abs_path('output')
    ...
    use_image_sequences: bool = False  # use preprocessed image sequences rather than driving videos
    save_warped_features: bool = False  # whether to save warped features
    save_driving_kps: bool = False # whether to save the driving keypoints
    driving_image_folder = ''
    
    # finetuned_spade_ckpt = ''   # using my own check point for the spade decoder
    ...
    # ======for AmecaNeutralLarge===== Penny NOTE: for video
    scale: float = 1.97 # 2.3  # the ratio of face area is smaller if scale is larger
    vy_ratio: float = -0.18 # -0.125  # the ratio to move the face to up or down in cropping space
    # ================================
    # ======for real ameca_neutral.png=====
    # scale: float = 1.6 # 2.3  # the ratio of face area is smaller if scale is larger
    # vy_ratio: float = -0.18
    # ================================  # Penny NOTE: for images
    # scale: float = 2.3
    # vy_ratio = -0.125
    ...
    vy_ratio_crop_driving_video: float = 0.04 # -0.1  # adjust x offset postive: up; negative: down
